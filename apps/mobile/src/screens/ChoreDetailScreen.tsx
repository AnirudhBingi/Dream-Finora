...
export function ChoreDetailScreen({
  ...
  // Load rotation schedule separately (optional, conditional)
  useEffect(() => {
    async function loadRotationSchedule() {
      if (!chore || !chore.isRecurring || !chore.rotationEnabled || !token) {
        setRotationSchedule([]);
        return;
      }
  ...
  }, [chore?.isRecurring, chore?.rotationEnabled, choreId, token]);

  const { execute: handleGrab, loading: grabbing } = useAsyncOperation({
    operationFn: async () => {
      if (!token) throw new Error("Not authenticated");
      return grabChore(token, choreId);
    },
    onSuccess: () => {
  ...
  const { execute: handleAssign, loading: assigning } = useAsyncOperation({
    operationFn: async (userId: string) => {
      if (!token) throw new Error("Not authenticated");
      return assignChore(token, choreId, userId);
    },
    onSuccess: () => {
  ...
  const { execute: handleCompleteOperation, loading: completing } =
    useAsyncOperation({
      operationFn: async () => {
        if (!token) throw new Error("Not authenticated");
        return completeChore(token, choreId);
      },
      onSuccess: (result) => {
        const pointsEarned =
          result.lastCompletion?.pointsEarned || result.points;
        Alert.alert(
          "Success",
  ...
  function handleComplete() {
    Alert.alert(
      ...
      [
        ...
        {
          text: "Complete",
          onPress: () => handleCompleteOperation(),
        },
      ],
    );
  }
  ...
  const { execute: handleDeleteOperation, loading: deleting } =
    useAsyncOperation({
      operationFn: async () => {
        if (!token) throw new Error("Not authenticated");
        return deleteChore(token, choreId);
      },
      onSuccess: () => {
  ...
  const { execute: handleCompleteAssignment, loading: completingAssignment } =
    useAsyncOperation({
      operationFn: async (assignmentId: string) => {
        if (!token) throw new Error("Not authenticated");
        return completeChoreAssignment(token, choreId, assignmentId);
      },
      onSuccess: () => {
  ...
  function getUserDisplayName(
    user: Chore["createdByUser"] | Chore["assignedToUser"],
  ): string {
  ...
  function getStatusColor(status: Chore["status"]): string {
    switch (status) {
      ...
      case "assigned":
        return theme.colors.primary; // Indigo-500
      case "completed":
  ...
  const hasMyAssignment =
    chore.assignmentType === "multiple" &&
    chore.assignments?.some((a) => a.userId === user?.id && !a.completedAt);
  const canGrab =
    chore.status === "pending" &&
    !chore.assignedTo &&
    chore.assignmentType !== "multiple";
  const canComplete =
    (chore.status === "assigned" && isAssignedToMe) || hasMyAssignment;
  const canAssign =
    chore.status === "pending" ||
    (chore.status === "assigned" && chore.createdBy === user?.id);
            ...
            <View style={styles.heroContent}>
              <View style={styles.heroIconContainer}>
                <MaterialIcons
                  name={
                    getChoreCategoryMaterialIcon(
                      detectedCategory || chore.category,
                    ) as any
                  }
                  size={40}
              ...
              <View style={styles.quickInfoItem}>
                ...
                <View style={styles.quickInfoTextContainer}>
                  ...
                  <View style={styles.quickInfoValueRow}>
                    ...
                    {canGrab && (
                      <Text style={styles.quickInfoBonus}>
                        +{bonusPoints} bonus
                      </Text>
                    )}
                  </View>
                  {chore.rotationEnabled &&
                    ...
                    chore.rotation.length > 0 && (
                      <Text style={styles.quickInfoRotationNote}>
                        Each person gets {chore.points} full points when it's
                        their turn
                      </Text>
                    )}
                </View>
              </View>
              {chore.dueDate && <View style={styles.quickInfoDivider} />}
              {chore.dueDate && (
                <View style={styles.quickInfoItem}>
                  <MaterialIcons
                    name="event"
                    size={20}
                    color={
                      new Date(chore.dueDate) < new Date() && !chore.completedAt
                        ? theme.colors.error
                ...
                <View style={styles.quickInfoItem}>
                  ...
                  <View style={styles.quickInfoTextContainer}>
                    ...
                    <Text style={styles.quickInfoValue}>
                      {getUserDisplayName(chore.assignedToUser)}
                    </Text>
                  </View>
                </View>
              </>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.detailRow}>
              ...
              <View style={styles.userContainer}>
                <Avatar
                  avatarUrl={getAvatarUrl(
                    chore.createdByUser?.profile?.avatarUrl || null,
                  )}
                  displayName={getUserDisplayName(chore.createdByUser)}
                  size={24}
                />
                <Text style={styles.detailValue}>
                  {getUserDisplayName(chore.createdByUser)}
                </Text>
              </View>
            </View>
            ...
            {chore.friendUser && (
              <View style={styles.detailRow}>
                ...
                <View style={styles.userContainer}>
                  <Avatar
                    avatarUrl={getAvatarUrl(
                      chore.friendUser?.profile?.avatarUrl || null,
                    )}
                    displayName={
                      chore.friendUser.profile?.displayName ||
                      chore.friendUser.email
                    }
                    size={24}
                  />
                  <Text style={styles.detailValue}>
                    {chore.friendUser.id === user?.id
                      ? "You"
                      : chore.friendUser.profile?.displayName ||
                        chore.friendUser.email}
                  </Text>
            ...
            {chore.assignmentType === "multiple" &&
              ...
              chore.assignments.length > 0 && (
                <View style={styles.assignmentsCard}>
                  <View style={styles.assignmentsCardHeader}>
                    <View style={styles.assignmentsCardHeaderLeft}>
                      <MaterialIcons
                        ...
                        color={theme.colors.primary}
                      />
                      <Text style={styles.assignmentsCardTitle}>
                        Assigned Members
                      </Text>
                    </View>
                    <Text style={styles.assignmentsCardCount}>
                  ...
                  {/* Progress Summary */}
                  {(() => {
                    const completedCount = chore.assignments.filter(
                      (a) => !!a.completedAt,
                    ).length;
                  ...
                  >
                    {chore.assignments.map((assignment) => {
                      ...
                      const displayName =
                        assignment.user?.profile?.displayName ||
                      ...
                      return (
                        ...
                        >
                          <View style={styles.assignmentMemberAvatarWrapper}>
                            ...
                            {isCompleted && (
                              <View
                                style={styles.assignmentMemberCompletedBadge}
                              >
                                <MaterialIcons
                                  name="check-circle"
                                  size={18}
                            ...
                            {isMyAssignment && !isCompleted && (
                              <View style={styles.assignmentMemberYouBadge}>
                                <Text
                                  style={styles.assignmentMemberYouBadgeText}
                                >
                                  You
                                </Text>
                              </View>
                          ...
                          <Text
                            style={[
                              ...
                              isCompleted &&
                                styles.assignmentMemberNameCompleted,
                            ]}
                            numberOfLines={1}
                          >
                ...
                {chore.completions.map((completion, index) => (
                  <View
                    ...
                    style={[
                      ...
                      index < chore.completions!.length - 1 &&
                        styles.completionItemBorder,
                    ]}
                  >
                    <View style={styles.completionUserInfo}>
                      <Avatar
                        avatarUrl={getAvatarUrl(
                          completion.user?.profile?.avatarUrl || null,
                        )}
                        displayName={
                          completion.user?.profile?.displayName ||
                  ...
                  <View style={styles.scheduleItemContent}>
                    <Text style={styles.scheduleItemValue}>
                      {chore.recurrencePattern
                        ? chore.recurrencePattern.charAt(0).toUpperCase() +
                          chore.recurrencePattern.slice(1)
                        : "Custom"}
                    </Text>
                  ...
                  <View style={styles.scheduleItem}>
                    ...
                    <View style={styles.scheduleItemContent}>
                      ...
                      {chore.rotation && chore.rotation.length > 0 && (
                        <Text style={styles.scheduleItemSubtext}>
                          {chore.rotation.length} member
                          {chore.rotation.length !== 1 ? "s" : ""} in rotation
                        </Text>
                      )}
                      {!chore.isRecurring && (
                        <Text
                          style={[
                            styles.scheduleItemSubtext,
                            { color: theme.colors.warning, marginTop: 4 },
                          ]}
                        >
                          ⚠️ Rotation works best with recurring tasks. For
                          one-time tasks, it only assigns to the first person.
                        </Text>
                      )}
                  ...
                  <View style={styles.rotationList}>
                    {chore.rotation
                      ...
                      .map((rotationMember, index) => {
                        const isSkipped =
                          rotationMember.skipUntil &&
                          new Date(rotationMember.skipUntil) > new Date();
                        const lastAssignedDate = rotationMember.lastAssignedAt
                          ? new Date(
                              rotationMember.lastAssignedAt,
                            ).toLocaleDateString("en-US", {
                              month: "short",
                              day: "numeric",
                              year: "numeric",
                        ...
                        return (
                          ...
                          >
                            <View style={styles.rotationItemLeft}>
                              ...
                              <Avatar
                                avatarUrl={getAvatarUrl(
                                  rotationMember.user?.profile?.avatarUrl ||
                                    null,
                                )}
                                displayName={
                                  rotationMember.user?.profile?.displayName ||
                                  rotationMember.user?.email ||
                                  "Unknown"
                                }
                                size={32}
                              />
                  ...
                  <View style={styles.card}>
                    ...
                    {rotationScheduleLoading ? (
                      <ActivityIndicator
                        size="small"
                        color={theme.colors.primary}
                        style={styles.scheduleLoading}
                      />
                    ) : (
                      <View style={styles.scheduleList}>
                        {rotationSchedule.slice(0, 10).map((item, index) => {
                          const assignedMember = members.find(
                            (m) => m.userId === item.assignedToUserId,
                          );
                          ...
                          return (
                            <View
                              ...
                              style={[
                                ...
                                isPast &&
                                  !item.isAssigned &&
                                  styles.scheduleItemPast,
                              ]}
                            >
                              <View style={styles.scheduleItemLeft}>
                                <View
                                  style={[
                                    styles.scheduleNumberBadge,
                                    isMyTurn &&
                                      styles.scheduleNumberBadgeHighlighted,
                                  ]}
                                >
                                  <Text
                                    style={[
                                      ...
                                      isMyTurn &&
                                        styles.scheduleNumberTextHighlighted,
                                    ]}
                                  >
      ...
      {!chore.completedAt && (
        <View style={styles.actionButtonsContainer}>
          ...
          {chore.assignmentType === "multiple" &&
            hasMyAssignment &&
            (() => {
              const myAssignment = chore.assignments?.find(
              ...
              return (
                <TouchableOpacity
                  style={[styles.actionButton, styles.completeButton]}
                  onPress={() => handleCompleteAssignment(myAssignment.id)}
