...
export function SpaceVListScreen({
  onCreateSpaceV,
  onViewSpaceV,
  onBack,
}: SpaceVListScreenProps) {
  const { token } = useAuth();
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['left', 'right']}>
      ...
      >
        <View style={styles.header}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            accessibilityRole="button"
            accessibilityLabel="Create SpaceV"
            accessibilityHint="Create a new SpaceV listing"
          >
            <Text style={styles.createButtonText}>List Item</Text>
          </TouchableOpacity>
        </View>
      ...
      {/* Create spacer interpolation directly from scrollY (perfectly synchronized) */}
      <Animated.View 
        style={{
          height: scrollY.interpolate({
            inputRange: [-200, 0, SCROLL_THRESHOLD, SCROLL_THRESHOLD + HIDE_DISTANCE, 10000],
      ...
      <Animated.ScrollView
        ...
        onScrollEndDrag={(e) => {
          // Check pagination when user stops dragging
        ...
        onMomentumScrollEnd={(e) => {
          // Also check when momentum scrolling ends
          const { layoutMeasurement, contentOffset, contentSize } = e.nativeEvent;
          const paddingToBottom = 20;
          if (layoutMeasurement.height + contentOffset.y >= contentSize.height - paddingToBottom) {
            if (hasMore && !loadingMore) {
      ...
      >
        <View style={styles.content}>
          ...
          <Text style={styles.sectionTitle}>SpaceV</Text>

          {listings.length === 0 ? (
            <EmptyState
              icon="list"
              title="No SpaceV listings found"
              message={searchQuery || selectedType
                ? 'Try adjusting your search or filters'
                : 'Create your first SpaceV listing to get started!'}
              actionLabel={!searchQuery && !selectedType ? 'Create SpaceV' : undefined}
              onAction={!searchQuery && !selectedType ? onCreateSpaceV : undefined}
            />
          ) : (
            <>
              ...
              <TouchableOpacity
                ...
                onPress={() => onViewSpaceV(listing.id)}
                activeOpacity={0.7}
              >
                ...
                <View style={styles.listingContent}>
                  ...
                  <View style={styles.listingFooter}>
                    ...
                    <Text style={styles.listingViews}>{listing.views || 0} views</Text>
                  </View>
                </View>
              </TouchableOpacity>
              ))}
