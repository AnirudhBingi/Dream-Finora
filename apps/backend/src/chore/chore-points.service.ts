...
export class ChorePointsService {
  ...
  calculatePoints(
    ...
    const basePoints =
      category && this.categoryBasePoints[category]
    ...
    let calculatedPoints = Math.round(
      basePoints * maxEffortMultiplier * timeMultiplier,
    );
  ...
  getPointExplanation(
    ...
    if (category && this.categoryBasePoints[category]) {
      explanations.push(
        `Base: ${this.categoryBasePoints[category]} pts (${category})`,
      );
    }
    ...
    if (effortKeywords.length > 0) {
      ...
      if (multiplier > 1.2) {
        explanations.push(
          `High effort: ${keyword} (${(multiplier * 100).toFixed(0)}%)`,
        );
      } else if (multiplier < 0.9) {
        explanations.push(
          `Low effort: ${keyword} (${(multiplier * 100).toFixed(0)}%)`,
        );
      }
    }
    ...
    if (timeKeywords.length > 0) {
      ...
      if (multiplier !== 1.0) {
        explanations.push(
          `Time: ${keyword} (${(multiplier * 100).toFixed(0)}%)`,
        );
      }
    }
  ...
  async calculatePointsEarned(
    ...
    const breakdown: {
      ...
      penaltyAmount?: number;
      finalPoints: number;
    } = { basePoints, finalPoints: basePoints };
