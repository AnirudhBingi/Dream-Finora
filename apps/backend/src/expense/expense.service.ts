...
type ExpenseSplitWithUser = Prisma.ExpenseSplitGetPayload<{
  include: {
    User: {
      select: {
        id: true;
...
type ExpenseHistoryWithUser = Prisma.ExpenseHistoryGetPayload<{
  include: {
    User: {
      select: {
        id: true;
...
export class ExpenseService {
  ...
  private transformExpense(expense: ExpenseWithRelations) {
    const {
      ExpenseSplit,
      User_Expense_createdByToUser,
      User_Expense_paidByToUser,
    ...
    return {
      ...
      splits: (ExpenseSplit || []).map((split: ExpenseSplitWithUser) => ({
        id: split.id,
      ...
      createdByUser: User_Expense_createdByToUser
        ? {
            ...
            profile: User_Expense_createdByToUser.UserProfile
              ? {
                  displayName:
                    User_Expense_createdByToUser.UserProfile.displayName,
      ...
      paidByUser: User_Expense_paidByToUser
        ? {
            ...
            profile: User_Expense_paidByToUser.UserProfile
              ? {
                  displayName:
                    User_Expense_paidByToUser.UserProfile.displayName,
      ...
      group: Group
        ...
        : null,
      // Include ride summary if expense was created from a ride
      ride: Ride
        ? {
            id: Ride.id,
            origin: Ride.origin,
  ...
  async createExpense(userId: string, createExpenseDto: CreateExpenseDto) {
    console.log(
      '[ExpenseService] Creating expense with groupId:',
    ...
    if (!expenseParticipantIds.includes(paidBy)) {
      throw new BadRequestException(
        'PaidBy user must be a participant in the expense',
      );
    }
    ...
    console.log('[ExpenseService] Creating expense with splits:', {
      ...
      splits: createExpenseDto.splits.map((s) => ({
        userId: s.userId,
        amount: s.amount,
      })),
    });
    ...
    console.log(
      '[ExpenseService] Expense created with groupId:',
      expense.groupId,
      'Group:',
      expense.Group,
    );
    ...
    const participantIds = expense.ExpenseSplit.map(
      (split) => split.userId,
      ...
      expense.ExpenseSplit.map(async (split) => {
        if (split.amount > 0) {
          ...
          try {
            ...
            console.error(
              `[ExpenseService] Failed to sync split ${split.id} to finance:`,
              err,
            );
  ...
  async getExpenses(userId: string, limit: number = 50, offset: number = 0) {
    ...
    const [expenses, total] = await Promise.all([
      this.prisma.expense.findMany({
        where: {
          OR: [{ createdBy: userId }, { ExpenseSplit: { some: { userId } } }],
        },
        include: {
          ExpenseSplit: {
            include: {
  ...
  async markSplitAsPaid(userId: string, expenseId: string, splitId: string) {
    ...
    if (updated.Expense.createdBy !== userId) {
      ...
      const paidByName =
        payer?.UserProfile?.displayName || payer?.email || 'Someone';
      await this.notificationService
        ...
        .catch((err) => {
          console.error(
            `Failed to create notification for expense creator:`,
            err,
          );
        });
    }
  ...
  async updateReceipt(userId: string, expenseId: string, receiptUrl: string) {
    ...
    if (!expense) {
      throw new NotFoundException(
        'Expense not found or you do not have permission',
      );
    }

    const updated: ExpenseWithRelations = await this.prisma.expense.update({
      where: { id: expenseId },
      data: { receiptUrl },
      include: {
        ExpenseSplit: {
  ...
  async updateExpense(
    ...
    if (updateExpenseDto.amount !== undefined || updateExpenseDto.splits) {
      ...
      if (updateExpenseDto.splits) {
        ...
        const oldTotal = expense.ExpenseSplit.reduce(
          (sum, split) => sum + split.amount,
          0,
        );
        if (oldTotal > 0) {
          ...
          const totalRecalculated = recalculatedSplits.reduce(
            (sum, split) => sum + split.amount,
            0,
          );
          const difference = newAmount - totalRecalculated;
          if (Math.abs(difference) > 0.01) {
            recalculatedSplits[0].amount =
              Math.round((recalculatedSplits[0].amount + difference) * 100) /
              100;
          }
        } else {
          // No existing splits - can't recalculate
          throw new BadRequestException(
            'Cannot update amount: expense has no splits to recalculate',
          );
        }
      } else {
      ...
      const totalSplitAmount = recalculatedSplits.reduce(
        (sum, split) => sum + split.amount,
        0,
      );
    ...
    if (
      ...
      changes.description = {
        ...
        after: updateExpenseDto.description,
      };
    }
    if (
      updateExpenseDto.amount !== undefined &&
      ...
      changes.amount = {
        ...
        after: updateExpenseDto.amount,
      };
    }
    if (
      updateExpenseDto.currency !== undefined &&
      updateExpenseDto.currency !== expense.currency
    ) {
      changes.currency = {
        before: expense.currency,
        after: updateExpenseDto.currency,
      };
    }
    ...
    if (updateExpenseDto.date !== undefined) {
      changes.date = {
        before: expense.date,
        after: new Date(updateExpenseDto.date),
      };
    }
    if (recalculatedSplits) {
      changes.splits = {
        before: expense.ExpenseSplit.map((s) => ({
          userId: s.userId,
          amount: s.amount,
        })),
        after: recalculatedSplits,
      };
    }

    // If splits need to be updated (either provided or recalculated), delete old splits and create new ones
    if (recalculatedSplits) {
      const splitsToCreate = recalculatedSplits;
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        // Update expense
        const updated: ExpenseWithRelations = await tx.expense.update({
          where: { id: expenseId },
          data: {
            ...updateData,
            ExpenseSplit: {
              create: splitsToCreate.map((split) => ({
                id: randomUUID(),
                userId: split.userId,
                amount: split.amount,
  ...
  async deleteExpense(userId: string, expenseId: string) {
    ...
    if (expenseWithSplits) {
      const deleterName =
        expenseWithSplits.User_Expense_createdByToUser.UserProfile
          ?.displayName || expenseWithSplits.User_Expense_createdByToUser.email;
      const participantIds = expenseWithSplits.ExpenseSplit.map(
        (split) => split.userId,
      ...
      await Promise.all(
        expenseWithSplits.ExpenseSplit.map(async (split) => {
          try {
            ...
            console.error(
              `[ExpenseService] Failed to delete finance transaction for split ${split.id}:`,
              err,
            );
    ...
    if (linkedRide) {
      await this.prisma.ride
        ...
        .catch((err) => {
          console.error(
            `[ExpenseService] Failed to unlink ride ${linkedRide.id} from deleted expense:`,
            err,
          );
        });
    }
  ...
  async getExpenseHistory(userId: string, expenseId: string) {
    ...
    // Get history entries
    const history: ExpenseHistoryWithUser[] =
      await this.prisma.expenseHistory.findMany({
        where: {
          expenseId: expenseId,
        },
        include: {
    ...
    try {
      ...
      const convertSplits = async (
        ...
        const validSplits = splits.filter((split) => {
          if (!split.Expense) {
            console.warn(
              `[ExpenseService] Skipping orphaned split ${split.id} - expense not found`,
            );
            return false;
          }
          return true;
      ...
      const filteredOwedToCount =
        owedToUser.length - convertedOwedToUser.length;
      if (filteredOwedCount > 0 || filteredOwedToCount > 0) {
        console.warn(
          `[ExpenseService] Filtered out ${filteredOwedCount + filteredOwedToCount} orphaned splits (missing expenses)`,
        );
      }
      ...
      const validOwedToUserSplits = convertedOwedToUser.filter(
        (s) => s.Expense,
      );
      const expenseIds = [
        ...new Set(
          [...validOwedSplits, ...validOwedToUserSplits].map(
            (s) => s.Expense.id,
          ),
        ),
      ];
      const ridesByExpenseId = new Map<
        string,
      ...
      const owedByUser = new Map<
        string,
        {
          ...
          breakdown: {
            byGroup: Map<string, { groupName: string; amount: number }>;
            rideshare: number;
            individual: number;
          };
        }
      ...
      convertedOwedSplits.forEach((split) => {
        ...
        if (!split.Expense) {
          console.warn(
            '[ExpenseService] Skipping split with missing expense:',
            split.id,
          );
          return;
        }
        ...
        if (!owedByUser.has(creditorId)) {
          ...
          if (!creditorUser) {
            console.error(
              '[ExpenseService] Missing creditor user for expense:',
              split.Expense.id,
              'creditorId:',
              creditorId,
            );
      ...
      convertedOwedToUser.forEach((split) => {
        ...
        if (!debtorId) {
          console.warn(
            '[ExpenseService] Missing debtorId for split:',
            split.id,
          );
        ...
        if (!owedToUserByUser.has(debtorId)) {
          if (!split.User) {
            console.error(
              '[ExpenseService] Missing User for split:',
              split.id,
              'debtorId:',
              debtorId,
            );
      ...
      console.log('[ExpenseService] Balance results:', {
        ...
        owedByUserDetails: Array.from(owedByUser.entries()).map(
          ([id, data]) => ({
            userId: id,
            userName:
              data.user?.UserProfile?.displayName ||
              data.user?.email ||
              'Unknown',
            amount: data.amount,
        ...
        owedToUserDetails: Array.from(owedToUserByUser.entries()).map(
          ([id, data]) => ({
            userId: id,
            userName:
              data.user?.UserProfile?.displayName ||
      ...
      return {
        ...
        owedByUser: Array.from(owedByUser.values()).map((item) => ({
          ...
          breakdown: {
            byGroup: Array.from(item.breakdown.byGroup.entries()).map(
              ([groupId, groupData]) => ({
                groupId,
      ...
      throw new BadRequestException(
        'Failed to calculate balances. Please try again later.',
      );
    }
  }
  ...
  async createSettlement(
    ...
    const expenseWhereClause: Prisma.ExpenseWhereInput = {
      OR: [
        ...
        {
          AND: [{ paidBy: null }, { createdBy: payeeId }],
        },
      ],
    };
    ...
    const expenseWhereClauseOpposite: Prisma.ExpenseWhereInput = {
      OR: [
        { paidBy: payerId }, // Payer paid for the expense
        {
          AND: [{ paidBy: null }, { createdBy: payerId }],
        },
      ],
    };
    ...
    if (splitsPayerOwesPayee.length === 0 && splitsPayeeOwesPayer.length > 0) {
      ...
      console.warn(
        '[ExpenseService] Settlement direction mismatch: payeeId is receiver but no splits found in that direction',
      );
    ...
    if (
      createSettlementDto.splitIds &&
      createSettlementDto.splitIds.length > 0
    ) {
    ...
    const totalOwed =
      Math.round(
        splitsToSettle.reduce((sum, split) => sum + split.amount, 0) * 100,
      ) / 100;
    ...
    const splitsToPartiallyPay: Array<{
      splitId: string;
      paidAmount: number;
      remainingAmount: number;
    }> = []; // Splits with partial payment
    ...
    for (const split of splitsToSettle) {
      ...
      if (roundedAmount >= splitAmount - 0.01) {
        ...
        const remainingSplitAmount =
          Math.round((splitAmount - roundedAmount) * 100) / 100;
    ...
    const settlement = await this.prisma.$transaction(async (tx) => {
      ...
      const allPaidSplitIds = [
        ...splitsToFullyPay,
        ...splitsToPartiallyPay.map((s) => s.splitId),
      ];
    ...
    await this.trustScoreService.updateExpenseScore(payerId).catch((err) => {
      console.error(
        'Failed to update trust score for payer after settlement:',
        err,
      );
    });
    await this.trustScoreService.updateExpenseScore(payeeId).catch((err) => {
      console.error(
        'Failed to update trust score for payee after settlement:',
        err,
      );
    });
    ...
    const payerName =
      ...
      settlement.User_Settlement_payerIdToUser.email;
    const expenseId =
      settlement.SettlementSplit[0]?.ExpenseSplit?.Expense?.id || '';
    await this.notificationService
  ...
  async simplifyDebts(userId: string) {
    ...
    balances.owedByUser.forEach((entry) => {
      debtGraph.set(
        entry.user.id,
        (debtGraph.get(entry.user.id) || 0) + entry.amount,
      );
    });
    ...
    return {
      ...
      simplifiedDebts: transactions.map((t) => {
        ...
        if (!fromUser || !toUser) {
          throw new NotFoundException(
            'User not found in simplified debt calculation',
          );
        }
        return {
